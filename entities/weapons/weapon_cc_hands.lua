		...
		if tr.Entity and tr.Entity:IsValid() then
			...
			if GAMEMODE.FistsHaveEffectOnPlayers then
				...
				if tr.Entity:IsPlayer() then
					...
					local dmg = (5 + self.Owner:RemapStatStrength(0, 5)) * blockmul

					tr.Entity:TakeCDamage(math.floor(dmg)/2)
...
function SWEP:Reload()
	...
	if tr.Entity and tr.Entity:IsValid() and (tr.Entity:GetClass() == "prop_door_rotating" or (tr.Entity:GetClass() == "func_door_rotating" and tr.Entity:DoorType() ~= DOOR_COMBINEOPEN)) and tr.Fraction > 0.2 then
		...
		if self.Owner:RemapStatStrength(0, 1) < 0.4 then
			if CLIENT then
				GAMEMODE:AddChat("You're not strong enough to ram this door!", Color(200, 0, 0, 255))
			end
		...
		if SERVER then
			...
			local n = math.ceil(10 - self.Owner:RemapStatStrength(0, 8))

			if not tr.Entity:GetSaveTable().m_bLocked then
				n = math.ceil(2 - self.Owner:RemapStatStrength(0, 1))
			end

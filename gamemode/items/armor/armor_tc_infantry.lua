...
ITEM.Name 					= "Tech-Com Infantry Uniform"
ITEM.Description 			= "A Tech-Com uniform outfitted with standard armor plates and ammo pouches, designed for use by general infantry."
...
if SERVER then
	function ITEM:GetModelData(ply, data)
		local mdl = self:GetProperty("Casual") and "models/tnb/clothing/trp/body/%s_tc_offduty.mdl" or "models/tnb/clothing/trp/body/%s_tc_infantry.mdl"
		...
		data.body = {
			...
			bodygroups = {
				...
				strap = not self:GetProperty("Strap"),
...
function ITEM:GetSecondaryOptions(ply, tab)
	...
	if not casual then
		table.insert(tab, {
			...
			Func = function(item, user)
				if SERVER then
					self:SetProperty("Strap", not self:GetProperty("Strap"))

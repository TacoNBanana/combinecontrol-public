	...
	OnParsed = function(t)
		...
		if IsValid(ent) and ent:GetClass() == "cc_radio" and ply:GetPos():Distance(ent:GetPos()) <= 256 then
			if not ent:GetEnabled() then
				ply:SendChat(nil, "ERROR", "You need to turn it on, first!")
		...
		for _, v in pairs(ents.FindByClass("cc_radio")) do
			if v:GetChannel() ~= freq or not v:GetEnabled() then
				continue
			end
...
GM:AddChatCommand({
	...
	OnParsed = function(t)
		...
		if not ply:HasItem("phone") then
			ply:SendChat(nil, "WARNING", "You tap at the palm of your hand.")

			timer.Simple(1.5, function()
				ply:SendChat(nil, "WARNING", "9-1-1.")
			end)

			timer.Simple(2.5, function()
				ply:SendChat(nil, "WARNING", "You wait...")
			end)

			timer.Simple(4, function()
				ply:SendChat(nil, "WARNING", "...and wait...")
			end)
			...
			timer.Simple(8, function()
				ply:SendChat(nil, "ERROR", "Nothing happens.")
			end)
		...
		local class = GAMEMODE.MessageTypes.SAY
		local radclass = GAMEMODE.MessageTypes.REQUEST
		...
		for _, v in pairs(player.GetAll()) do
			...
			local radio = v:GetEquipment(EQUIPMENT_RADIO)

			if not radio then
				continue
			end

			-- hardcoded is prob fine for the temporary period
			if radio:GetClass() == "radio_police" then
				table.insert(rem, v)
				seen[v] = true
			end
		end
...
GM:AddChatCommand({
	...
	OnParsed = function(t)
		...
		if ply:Money() < 10 then
			ply:SendChat(nil, "ERROR", "You need 10 dollars to make an advertisement.")
			return
		end

...
net.Receive("nChat", function(len, ply)
	...
	if not class then
		...
		class = GAMEMODE.ChatCommands[string.lower(cmd)]
		if not class then
			GAMEMODE:SendChat(nil, ply, "ERROR", "Unknown chat command '" .. string.lower(cmd) .. "'!")
			return
		end
	end
...
GM:AddChatCommand({
	...
	OnParsed = function(t)
		...
		if cmd == "/rcom" and (not item or not item:GetProperty("AllowCommand")) then
			GAMEMODE:SendChat(nil, ply, "ERROR", "Unknown chat command '" .. cmd .. "'!")
			return
		end
		...
		elseif item then
			...
			local available, msg = item:ChannelAvailable(freq)

			if not available then
				ply:SendChat(nil, "ERROR", msg or string.format("Your %s isn't transmitting.", item:GetName()))
		...
		if ply:Team() != TEAM_SKYNET and ply:Team() != TEAM_REPROG then
		...
		local jammed = GAMEMODE:GetRadioJamming(ply)

		if jammed > 0 and t.Custom.Freq < 1000 then
			text = string.Gibberish(text, jammed)
			t.Custom.Jammed = jammed
		end

		GAMEMODE:SendChat(ply, loc, class, t.Text, t.Language, true)
		GAMEMODE:SendChat(ply, rem, radclass, text, t.Language, false, t.Custom)

		t.Custom.Stationary = true

		GAMEMODE:SendChat(ply, static, radclass, text, t.Language, true, t.Custom)
	end
})
...
GM:AddChatCommand({
	...
	OnParsed = function(t)
		...
		if ply:CharFlags() != "S" then
			ply:SendChat(nil, "ERROR", "This command is only available to SkyNET drones.")

			return
		end

		if not ply:Alive() then
			ply:SendChat(nil, "ERROR", "You're dead.")

			return
		end

		local flags = ply:IsSuperAdmin() and GAMEMODE.DroneFlags or ply:DroneFlags() -- Bit of a hack but hey, it works

		if table.Count(flags) == 0 then
			ply:SendChat(nil, "ERROR", "You don't have any drone flags available. Contact an admin to fix this.")

			return
		end

		local arg = string.lower(t.Text)

		if not flags[arg] then
			ply:SendChat(nil, "WARNING", "Available flags:")

			for k in SortedPairs(flags) do
		...
		ply:SetActiveDroneFlag(arg)
		ply:UpdatePlayerField("ActiveDroneFlag", arg)

		GAMEMODE:PlayerCheckFlag(ply)
		GAMEMODE:PlayerLoadout(ply)
		GAMEMODE:PlayerUpdateName(ply)

		GAMEMODE:WriteLog("character_swapdrone", {

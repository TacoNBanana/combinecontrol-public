...
function GM:DrawPlayerInfo()
	...
	draw.RoundedBox(0, 20, ScrH() - 75, w, 56, Color(30, 30, 30, 200))
	...
	draw.DrawTextShadow(tostring(LocalPlayer():Money()) .. " Dollars", "CombineControl.LabelGiant", w + 20 - 4, ScrH() - 75 + 4 + 22 + 4, Color(200, 200, 200, 255), Color(0, 0, 0, 255), 2)
end
...
function GM:GetPlayerSight()
	...
	if self.FlashbangStart and CurTime() - self.FlashbangStart < 5 then
		return 0
	elseif IsValid(wep) and (wep.BinocularZoom or (wep.Scoped and wep.InSights and wep:InSights())) then
		return 5000
	else
	...
	for k, v in pairs(self.EntityTable.door) do
		...
		if v.HUDAlpha > 0 then
			...
			if (v:DoorType() == DOOR_BUYABLE or v:DoorType() == DOOR_BUYABLE_ASSIGNABLE) and #v:DoorOwners() == 0 and #v:DoorAssignedOwners() == 0 then
				draw.DrawTextShadow(v:DoorPrice() .. " dollars", "CombineControl.PlayerFont", pos.x, pos.y, Color(226, 205, 95, v.HUDAlpha * 255), Color(0, 0, 0, v.HUDAlpha * 255), 1)
				pos.y = pos.y + 20
			end
...
function GM:DrawAmmo()
	...
	if w != NULL then
		...
		if (w.Firearm or w.Tekka) and w.Primary.ClipSize > -1 then
			...
			if w.Firemodes then
				local firemode = w:GetFiremode().Name
				local ammo = GAMEMODE:GetBullet(w:GetAmmoType())

				local text = string.format("%s, %s", firemode, ammo and ammo.Name or "Unloaded")
...
function GM:HUDPaint()
	...
	if not self.CharCreate then
		...
		if cookie.GetNumber("cc_hud", 1) == 1 and not self.Mastermind then
			...
			if not LocalPlayer():TargetHUD() then
				self:DrawEntities()
				self:DrawPlayerInfo()
				self:DrawHealthBars()
			end
...
function GM:SetNVG(state)
	if not self.NVGActive then
		self.NVGActive = false
	end

	if self.NVGActive == state then
		return
	end
...
net.Receive("nSetNightvision", function(len)
	local state = tobool(net.ReadBit())

	GAMEMODE:SetNVG(state)
end)

net.Receive("nToggleNightvision", function(len)
	GAMEMODE:SetNVG(not GAMEMODE.NVGActive)
end)
...
function GM:DrawTargetHUD()
	...
	for _, v in pairs(ents.GetAll()) do
		...
		if not (v:IsNPC() or v:IsPlayer()) or v == LocalPlayer() then
			continue
		end

		local ent = self:ShouldDrawStencilEnt(v)

		if not ent then
			continue
		end

		ent.PixVis = ent.PixVis or util.GetPixelVisibleHandle()

		local vis = util.PixelVisible(ent:LocalToWorld(ent:OBBCenter()), ent:BoundingRadius(), ent.PixVis)
		local alpha = vis * self.TargetScale
		...
		table.insert(targets, {
			Entity = v,
			Distance = LocalPlayer():EyePos():Distance(v:GetPos()),
			Min = hudmin,
			Max = hudmax,
			Alpha = alpha
		})
	...
	for _, v in SortedPairsByMemberValue(targets, "Distance") do
		if not target and pos:WithinAABox(v.Min, v.Max) then
			target = v.Entity
		end

		local w = v.Max.x - v.Min.x
		local h = v.Max.y - v.Min.y
		local color = (v.Entity:IsPlayer() and v.Entity != target) and GAMEMODE:GetTeamColor(v.Entity) or Color(255, 255, 255)
...
function GM:DrawTargetHUDText(pos, target)
	...
	draw.DrawText("DIST: " .. math.Round(dist * 0.0254, 2), "DebugFixed", pos.x - size, pos.y + size, col)
...
hook.Add("OnTargetHUDChanged", "CL.Hud.TargetHUD", function(ply, bool)
	surface.PlaySound(bool and "items/nvg_on.wav" or "items/nvg_off.wav")

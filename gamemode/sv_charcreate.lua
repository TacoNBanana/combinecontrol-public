net.Receive("nCreateCharacter", function(len, ply)
	...
	for _, v in pairs(stats) do
		sum = sum + v - GAMEMODE.MinStats
	end

	local r, err = GAMEMODE:CheckCharacterValidity(name, desc, model, skin, stats, sum, trait)

	if r then

		ply:SaveNewCharacter(name, desc, model, skin, stats, trait)

...
function GM:PMCreateLoans()
	...
	local amountLabel = vgui.Create("DLabel", CCP.PlayerMenu.ContentPane)
		...
		function amountLabel:Update(loan)
			amountLabel:SetText(loan .. " dollars")
	...
	local guide = vgui.Create("DLabel", CCP.PlayerMenu.ContentPane)
		guide:SetText("Loans are a great way to get some starting capital. You can take out up to " .. GAMEMODE.MaxLoan .. " dollars.\nYou cannot delete any character with a loan out, to prevent abuse.")
		guide:SetPos(10, 344)
...
function GM:PMCreateTakeLoan()
	...
	local label = vgui.Create("DLabel", frame)
		...
		label:SetFont("CombineControl.LabelBig")
		label:PerformLayout()
	frame.Label = label
...
function GM:PMPopulateBusiness(filter)
	...
	for i in pairs(self.ItemClasses) do
		...
		ui.icon = vgui.Create("DModelPanel", ui)

		ui.icon:SetPos(0, 0)
		ui.icon:SetSize(64, 64)

		ui.icon:SetFOV(item.FOV)
		ui.icon:SetCamPos(item.CamPos)
		ui.icon:SetLookAt(item.LookAt)

		ui.icon:SetModel(item.Model)
		ui.icon:SetColor(item.Color)

		local ent = ui.icon:GetEntity()
		...
		function ui.icon:LayoutEntity() end

		local p = ui.icon.Paint

		function ui.icon:Paint(w, h)
		...
		ui.price:SetText(string.Comma(item.BuyPrice) .. " Dollars")
		...
		function ui.buy:Calculate(val)
			...
			if val < 1 or not GAMEMODE:CanBuyItem(i, LocalPlayer(), val) then
				self:SetDisabled(true)
			else
				self:SetDisabled(false)
			end

			ui.price:SetText(string.Comma(item.BuyPrice * val) .. " dollars")
		end
...
function GM:PMCreateBusiness()
	...
	for k, v in SortedPairs(self.BusinessLicenses, true) do
		if v[2] then
			local price = v[2]
			local canAfford = LocalPlayer():Money() >= v[2]
			...
			CCP.PlayerMenu.BusinessLicenses[k]:SetText(v[1] .. " - " .. tostring(price) .. " dollars")
...
function GM:PMCreateLoansList()
	...
	for _, v in pairs(player.GetAll()) do
		if v:Loan() > 0 and v != LocalPlayer() then
			CCP.Loans.List:AddLine(v:FormattedCID(), v:VisibleRPName(), v:Loan() .. " dollars").Player = v
		end
	end
...
function GM:PMCreateEditLoan()
	...
	if IsValid(CCP.CPLoans.SelectedPlayer) then
		CCP.Loans.Loan = vgui.Create("DFrame")
		CCP.Loans.Loan:SetSize(250, 114)
		...
		CCP.Loans.Loan.Player = CCP.CPLoans.SelectedPlayer
		...
		CCP.Loans.Loan.Label = vgui.Create("DLabel", CCP.Loans.Loan)
		CCP.Loans.Loan.Label:SetText("Dollars")
		CCP.Loans.Loan.Label:SetPos(120, 34)
		CCP.Loans.Loan.Label:SetSize(130, 30)
		CCP.Loans.Loan.Label:SetFont("CombineControl.LabelBig")
		CCP.Loans.Loan.Label:PerformLayout()
		...
		CCP.Loans.Loan.OK:SetPos(190, 74)
			...
			if loan - val >= 0 then
				...
				if IsValid(CCP.Loans.List) then
					...
					for _, v in pairs(player.GetAll()) do
						...
						if loan > 0 then
							CCP.Loans.List:AddLine(v:FormattedCID(), v:VisibleRPName(), loan .. " dollars").Player = v
						end
					end
				end
...
net.Receive("nLoanDeducted", function(len)
	...
	GAMEMODE:AddChat(ply:VisibleRPName() .. " reduced your loan by " .. amt .. " dollars.", Color(200, 200, 200, 255))
end)

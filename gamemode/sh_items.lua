...
function GM:RegisterItem(path)
	...
	if string.Left(path, 4) != "base" and not ITEM.__Internal then
...
GM:RegisterItem("items/base_armband.lua")
GM:RegisterItem("items/base_armor.lua")
GM:RegisterItem("items/base_vest.lua")
GM:RegisterItem("items/base_clothing_body.lua")
GM:RegisterItem("items/base_boxed.lua")
...
GM:RegisterFolder("items/food")
GM:RegisterFolder("items/deployables")
GM:RegisterFolder("items/lights")
GM:RegisterFolder("items/junk")
GM:RegisterFolder("items/throwables")
GM:RegisterFolder("items/armbands")
GM:RegisterFolder("items/parts")
GM:RegisterFolder("items/armor")
GM:RegisterFolder("items/vests")
GM:RegisterFolder("items/boxed")
...
function GM:LoadItem(id, classname, overrides, location, locationarg, deleted)
	if SERVER and not class.Exists(classname) then
		while self.ItemRemapping[classname] do
			classname = self.ItemRemapping[classname]
		end
		...
		GAMEMODE.SQL:Update("$items", {ItemClass = classname}, "id = ?", id, stub)
	end
...
function GM:UnloadItem(item)
	...
	hook.Run("CC.SH.UnloadItem", item)

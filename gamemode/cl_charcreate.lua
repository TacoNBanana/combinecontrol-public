...
function GM:CreateQuiz()
	...
	local frame = vgui.Create("DFrame")
		...
		y = y + 32
		frame:AddQuestion("This server is ______-themed roleplay.", {"Real life", "Zombie", "Star-Wars", "Doctor Who"}, 2)
...
function GM:CreateCharCreate()
	...
	for _, v in pairs(self.Stats) do
		...
		CCP.CharCreatePanel["StatBar" .. v].Stat = 1

		CCP.CharCreatePanel["StatBar" .. v].Update = function(self)
			self:SetProgress(self.Stat / GAMEMODE.MaxStats)
			self:SetProgressText(self.Stat .. "/" .. GAMEMODE.MaxStats)
		end

		CCP.CharCreatePanel["StatBar" .. v]:Update()
		...
		function a:Think()
			if not self:IsDown() or CurTime() < (self.NextThink or 0) then
				return
			end

			if CCP.CharCreatePanel["StatBar" .. v].Stat > GAMEMODE.MinStats then
				CCP.CharCreatePanel.StatBarRemain.Stat = CCP.CharCreatePanel.StatBarRemain.Stat + 1
				CCP.CharCreatePanel.StatBarRemain:Update()

				CCP.CharCreatePanel["StatBar" .. v].Stat = CCP.CharCreatePanel["StatBar" .. v].Stat - 1
				CCP.CharCreatePanel["StatBar" .. v]:Update()
			end
		...
		function b:Think()
			...
			if CCP.CharCreatePanel["StatBar" .. v].Stat < GAMEMODE.MaxAllocatedStats and CCP.CharCreatePanel.StatBarRemain.Stat > 0 then
				CCP.CharCreatePanel.StatBarRemain.Stat = CCP.CharCreatePanel.StatBarRemain.Stat - 1
				CCP.CharCreatePanel.StatBarRemain:Update()
	...
	CCP.CharCreatePanel.StatBarRemain.Stat = GAMEMODE.StatsAvailable

	function CCP.CharCreatePanel.StatBarRemain:Update()
		self:SetProgress(self.Stat / GAMEMODE.StatsAvailable)
		self:SetProgressText(self.Stat .. "/" .. GAMEMODE.StatsAvailable)
	end
	...
	function CCP.CharCreatePanel.OK:DoClick()
		...
		for _, v in pairs(GAMEMODE.Stats) do
			stats[v] = CCP.CharCreatePanel["StatBar" .. v].Stat
		...
		function b:DoClick()
			if CCP.CharSelectPanel.DeleteMode then
				if tonumber(v.Loan) and tonumber(v.Loan) > 0 then
					CCP.CharSelectPanel.BadChar:SetText("You can't delete a character with a loan.")
					return
				end
...
function GM:CreateCharDeleteCancel()
	if CCP.CharSelectPanel then
		...
		b:SetSize(180, 20)
		function b:DoClick()
			if not CCP.CharSelectPanel.DeleteMode then
...
function GM:CreateMOTD(force)
	if not self.MOTDText then return end

	if cookie.GetNumber("cc_motd", 1) == 1 or force then

...
function meta:SetPhysgunColor()
	...
	if self:PhysgunMode() then
		local col = team.GetColor(self:Team())

		vec = Vector(col.r, col.g, col.b):GetNormalized()
	end
...
function meta:LoadPlayer(data)
	...
	self:SetDonations(util.JSONToTable(data.Donations), true)
	self:SetPhysgunMode(tobool(data.PhysgunMode), true)

	self:SetInvSpaceBonus(tonumber(data.InvSpaceBonus), true)
end
...
function meta:PostLoadCharacter()
	...
	if not self:HasBadge(BADGE_BIRTHDAY) and os.date("!%m-%d") == "02-09" then
		...
		self:SendChat(nil, "INFO", "It's Gangleider's birthday today! You've been awarded " .. util.FormatCurrency(400) .. " and a badge!")

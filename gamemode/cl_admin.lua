...
net.Receive("nAListCharacters", function(len)
	...
	for _, v in pairs(tab) do
		...
		MsgC(Color(200, 200, 200, 255), string.format("CharID: %-" .. fieldsLen.id ..
			...
			"s\n",v.id, v.RPName, v.CID, v.CharFlags))
	end
end)
...
net.Receive("nACharacterData", function(len)
	...
	local stats = {"StatSpeed", "StatStrength", "StatToughness", "StatPerception", "StatAgility", "StatAim"}
	local combinedStats = "("
	...
	for k, v in pairs(stats) do
		...
		combinedStats = combinedStats .. math.Round(tab[v])
		tab[v] = nil
	end

	combinedStats = combinedStats .. ")"
	tab.Stats = combinedStats
...
net.Receive("nACharacterLookup", function(len)
	...
	for _, v in pairs(tab) do
		...
		MsgC(Color(200, 200, 200, 255), string.format("CharID: %-" .. fieldsLen.id ..
			...
			"s | CID: %-" .. fieldsLen.CID ..
			"s | Flags: %-" .. fieldsLen.CharFlags ..
			"s\n",v.id, v.RPName, v.SteamID, v.CID, v.CharFlags))
	end
end)
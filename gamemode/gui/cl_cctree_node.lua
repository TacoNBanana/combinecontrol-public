...
AccessorFunc(PANEL, "m_bulletColor", 			"BulletColor")
AccessorFunc(PANEL, "m_sIcon", 				"Icon")
AccessorFunc(PANEL, "m_drawIcon", 				"DrawIcon")
...
function PANEL:Init()
	...
	self.Label = vgui.Create("CCTree_Node_Button", self)
	...
	self:SetIconColor(Vector(255, 255, 255))
...
function PANEL:InternalDoClick()
	...
	if (not self.m_bDoubleClickToOpen || (SysTime() - self.fLastClick < 0.3)) then
...
function PANEL:AnimSlide(anim, delta, data)
	if (not self.ChildNodes) then anim:Stop() return end
...
function PANEL:FilePopulate(bAndChildren, bExpand)
	...
	if (not folder || not wildcard || not path) then return false end
...
function PANEL:PerformLayout()
	...
	self.Label:SetTextInset(self.Expander.x + self.Expander:GetWide() + 4 + 16 + 4, 0)

	if (not self.ChildNodes || not self.ChildNodes:IsVisible()) then
...
--[[---------------------------------------------------------
   Name: AddNode
-----------------------------------------------------------]]
function PANEL:AddNode(strName, icon)
	self:CreateChildNodes()

	local pNode = vgui.Create("CCTree_Node", self)
		pNode:SetText(strName)
		pNode:SetParentNode(self)
		pNode:SetRoot(self:GetRoot())
		pNode:SetIconColor(icon)
...
function PANEL:CleanList()
	for k, panel in pairs(self.Items) do
		...
		if (not IsValid(panel) || panel:GetParent() ~= self.pnlCanvas) then
			self.Items[k] = nil
		end
...
function PANEL:Paint(w, h)
	derma.SkinHook("Paint", "CCTreeNode", self, w, h)
end
...
-----------------------------------------------------------]]
function PANEL:Copy()
	local copy = vgui.Create("CCTree_Node", self:GetParent())
	copy:SetText(self:GetText())
	copy:SetIconColor(self:GetIconColor())
...
derma.DefineControl("CCTree_Node", "Tree Node", PANEL, "DPanel")
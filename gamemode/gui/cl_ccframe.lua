...
--	self:SetCursor("sizeall")
	...
	function self.btnClose:DoClick() self:GetParent():Close() end
	function self.btnClose:Paint(w, h) derma.SkinHook("Paint", "WindowCloseButton", self, w, h) end

	self.lblTitle = vgui.Create("DLabel", self)
	function self.lblTitle:UpdateColours(skin)

		if (self:GetParent():IsActive()) then return self:SetTextStyleColor(skin.Colours.Window.TitleActive) end

		return self:SetTextStyleColor(skin.Colours.Window.TitleInactive)
...
function PANEL:PerformLayout()
	...
	self.btnClose:SetSize(24, 16)

	self.lblTitle:SetPos(6, 0)
	self.lblTitle:SetSize(self:GetWide() - 24, 20)
end

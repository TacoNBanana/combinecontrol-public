...
function meta:RecalculatePlayerModel()
	...
	if flag and flag.ModelFunc then
		...
		bodyData.model = self:IsFemale() and Model("models/tnb/techcom/female_survivor.mdl") or Model("models/tnb/techcom/male_survivor.mdl")
	end
...
--merge head data for hair?
	...
	if GAMEMODE.DarkSkinnedModels[mdl] then
		...
		if IsValid(body) then
			for k, v in pairs(GetMaterials(body)) do

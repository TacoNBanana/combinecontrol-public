...
hook.Add("OnGamemodeLoaded", "Crafting", function()
	local files = file.Find(GAMEMODE.FolderName .. "/gamemode/crafting/*.lua", "LUA", "namedesc")

	if #files > 0 then
		for _, v in pairs(files) do
			AddCSLuaFile(GAMEMODE.FolderName .. "/gamemode/crafting/" .. v)
			include(GAMEMODE.FolderName .. "/gamemode/crafting/" .. v)
		end
	end
end)
...
function GM:CanSeeRecipe(items, entity, index)
	...
	if bit.band(vis, CRAFTVIS_FIRSTINGREDIENT) == CRAFTVIS_FIRSTINGREDIENT and not self:HasRecipeItem(items, tab.Ingredients[1]) then
		return false
	end

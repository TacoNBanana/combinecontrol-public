...
concommand.AddAdmin("rpa_givemoney", function(ply, targ, amt)
	...
	GAMEMODE:WriteLog("admin_money", {Admin = GAMEMODE:LogPlayer(ply), Ply = GAMEMODE:LogPlayer(targ), Char = GAMEMODE:LogCharacter(targ), Amount = amt})
...
concommand.AddAdmin("rpa_deleteitem", function(ply, id)
	...
	if item then
		...
		GAMEMODE.SQL:Query("DELETE FROM $items WHERE id = ?", id, cb)
	end
end, false, {TYPE_NUMBER})
...
end, true, {TYPE_ENTITY, TYPE_BOOL})

...
concommand.AddAdmin("rpa_droneflags", function(ply)
	ply:SendChat(nil, "WARNING", "Available drone flags:")

	for k, v in SortedPairs(GAMEMODE.DroneFlags) do
		ply:SendChat(nil, "WARNING", string.format("  %s - %s", k, v.PrintName))
	end
end)

concommand.AddAdmin("rpa_listdroneflags", function(ply, targ)
	local flags = targ:DroneFlags()

	if targ:IsSuperAdmin() then
		ply:SendChat(nil, "ERROR", "Error: This person has access to all the flags.")

		return
	end

	if table.Count(flags) == 0 then
		ply:SendChat(nil, "WARNING", ply:Nick() .. " has no drone flags.")

		return
	end

	ply:SendChat(nil, "WARNING", ply:Nick() .. "'s drone flags:")

	for k in SortedPairs(targ:DroneFlags()) do
		ply:SendChat(nil, "WARNING", string.format("  %s - %s", k, GAMEMODE:GetDroneFlag(k).PrintName))
	end
end, false, {TYPE_ENTITY})

concommand.AddAdmin("rpa_givedroneflag", function(ply, targ, flag)
	if targ:IsSuperAdmin() then
		ply:SendChat(nil, "ERROR", "Error: This person doesn't need to be given flags.")

		return
	end

	if not GAMEMODE.DroneFlags[flag] then
		ply:SendChat(nil, "ERROR", "Error: This flag doesn't exist.")
	...
	targ:SetDroneFlags(flags)
	targ:UpdatePlayerField("DroneFlags", util.TableToJSON(flags))

	ply:SendChat(nil, "WARNING", string.format("You've given %s flags to %s", GAMEMODE:GetDroneFlag(flag).PrintName, targ:Nick()))
	targ:SendChat(nil, "WARNING", string.format("%s has given you %s flags, use '/flag %s' on a SkyNET character to switch to it.", ply:Nick(), flag, flag))

	GAMEMODE:WriteLog("admin_droneflags", {
		Admin = GAMEMODE:LogPlayer(ply),
		Ply = GAMEMODE:LogPlayer(targ),
		Give = true,
		Flag = flag
	})
end, false, {TYPE_ENTITY, TYPE_STRING})

concommand.AddAdmin("rpa_takedroneflag", function(ply, targ, flag)
	if targ:IsSuperAdmin() then
		ply:SendChat(nil, "ERROR", "Error: You cannot take flags from this person.")
	...
	ply:SendChat(nil, "WARNING", string.format("You've taken %s flags from %s", GAMEMODE:GetDroneFlag(flag).PrintName, targ:Nick()))
	targ:SendChat(nil, "WARNING", string.format("%s has taken %s flags from you.", ply:Nick(), flag))

	if targ:CharFlags() == "S" and targ:ActiveDroneFlag() == flag then
		targ:SetActiveDroneFlag("")
		targ:UpdatePlayerField("ActiveDroneFlag", arg)

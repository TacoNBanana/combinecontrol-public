...
net.Receive("nCReceiveMoney", function(len)
	local amt = net.ReadFloat()
	local ply = net.ReadEntity()

	GAMEMODE:AddChat(ply:VisibleRPName() .. " gave you " .. amt .. " dollars.", Color(200, 200, 200, 255))
end)

net.Receive("nUnownedStove", function(len)
	GAMEMODE:AddChat("You don't own this stove! It's rude to mess with other people's stuff.", Color(200, 0, 0, 255))
end)
...
net.Receive("nCPattedDown", function(len)
	...
	function ui:Update()
		...
		if self.SelectedItem then
			...
			if inv[id] then
				...
				for _, v in pairs(self.InvScroll:GetCanvas():GetChildren()) do
					if v.ID == id then
						v:DoClick()
	...
	ui.InvTitle:SetSize(ui:GetWide() - 430 - 110, 22)
	ui.InvTitle:PerformLayout()

	ui.InvDesc = vgui.Create("DLabel", ui)
	ui.InvDesc:SetText("No item selected.")
	ui.InvDesc:SetPos(420, 274)
	ui.InvDesc:SetFont("CombineControl.LabelSmall")
	ui.InvDesc:SetSize(ui:GetWide() - 430, 14)
	ui.InvDesc:SetAutoStretchVertical(true)
	ui.InvDesc:SetWrap(true)
	ui.InvDesc:PerformLayout()
...
function GM:GetCCOptions(ent, dist)
	...
	if ent and ent:IsValid() then
		...
		if ent:IsDoor() then
			...
			if (ent:DoorType() == DOOR_BUYABLE or ent:DoorType() == DOOR_BUYABLE_ASSIGNABLE) and #ent:DoorOwners() == 0 and #ent:DoorAssignedOwners() == 0 and not LocalPlayer():HasTerminatorTeam() then
			...
			elseif (ent:DoorType() == DOOR_BUYABLE or ent:DoorType() == DOOR_BUYABLE_ASSIGNABLE) and table.HasValue(ent:DoorOwners(), LocalPlayer():CharID()) then
				...
				local option = {"Sell", function()
					...
					self:AddChat("You sold the door for 80% of its original value (" .. tostring(math.floor(ent:DoorPrice() * 0.8)) .. " dollars).", Color(229, 201, 98, 255))
		...
		elseif ent:GetClass() == "cc_item" then
			...
			if ent.Item and #ent.Item:GetProperty("Description") > 0 then

				local option = {"Examine", function()
					self:AddChat(ent.Item:GetProperty("Description"), Color(200, 200, 200, 255))
		...
		elseif ent:GetClass() == "cc_radio" then
			local option = {"Channel", function()
				...
				CCP.RadioSelector.Entry:SetCaretPos(#CCP.RadioSelector.Entry:GetValue())
				...
				function CCP.RadioSelector.OK:DoClick()
					...
					if val >= 0 then
						...
						if val <= 999 then
							...
							GAMEMODE:AddChat("You change the radio channel to " .. tostring(val) .. ".", Color(200, 200, 200, 255))
							...
							GAMEMODE:AddChat("Highest channel is 999.", Color(200, 0, 0, 255))
						end
					else
						GAMEMODE:AddChat("Lowest channel is 0.", Color(200, 0, 0, 255))
					end
		...
		elseif ent:GetClass() == "cc_vendingmachine" then
			local option = {"Examine", function()
				self:AddChat("Only Breen Water is in stock. The machine says they costs 7 dollars each.", Color(200, 200, 200, 255))
			end, nil, 100}
		...
		elseif ent:GetClass() == "cc_paper" then
			...
			table.insert(tab, option)
		elseif ent:GetClass() == "prop_physics" and ent:PropSaved() == 0 and (LocalPlayer():IsAdmin() or LocalPlayer():ToolTrust() == 2) then
			local hasPermission = LocalPlayer():SteamID() == ent:PropSteamID() or LocalPlayer():IsAdmin()
...
function GM:CCCreateGiveMoney()
	CCP.GiveMoney = vgui.Create("DFrame")
	CCP.GiveMoney:SetSize(250, 114)
	CCP.GiveMoney:Center()
	CCP.GiveMoney:SetTitle("Give Money")
	CCP.GiveMoney.lblTitle:SetFont("CombineControl.Window")
	CCP.GiveMoney:MakePopup()
	CCP.GiveMoney.PerformLayout = CCFramePerformLayout
	CCP.GiveMoney:PerformLayout()

	CCP.GiveMoney.Think = UIAutoClose

	CCP.GiveMoney.Entry = vgui.Create("DTextEntry", CCP.GiveMoney)
	CCP.GiveMoney.Entry:SetFont("CombineControl.LabelBig")
	CCP.GiveMoney.Entry:SetPos(10, 34)
	CCP.GiveMoney.Entry:SetSize(100, 30)
	CCP.GiveMoney.Entry:PerformLayout()
	CCP.GiveMoney.Entry:RequestFocus()
	CCP.GiveMoney.Entry:SetNumeric(true)
	CCP.GiveMoney.Entry:SetCaretPos(#CCP.GiveMoney.Entry:GetValue())

	CCP.GiveMoney.Label = vgui.Create("DLabel", CCP.GiveMoney)
	CCP.GiveMoney.Label:SetText("Dollars")
	CCP.GiveMoney.Label:SetPos(120, 34)
	CCP.GiveMoney.Label:SetSize(130, 30)
	CCP.GiveMoney.Label:SetFont("CombineControl.LabelBig")
	CCP.GiveMoney.Label:PerformLayout()

	CCP.GiveMoney.OK = vgui.Create("DButton", CCP.GiveMoney)
	CCP.GiveMoney.OK:SetFont("CombineControl.LabelSmall")
	CCP.GiveMoney.OK:SetText("OK")
	CCP.GiveMoney.OK:SetPos(190, 74)
	CCP.GiveMoney.OK:SetSize(50, 30)
	function CCP.GiveMoney.OK:DoClick()

		if not CCSelectedEnt or not CCSelectedEnt:IsValid() then return end

		local val = tonumber(CCP.GiveMoney.Entry:GetValue())

		if not val or math.floor(val) < 1 then

			CCP.GiveMoney:Remove()
			return
		...
		if LocalPlayer():Money() >= val then
			...
			net.Start("nCGiveMoney")
				net.WriteFloat(val)
				net.WriteEntity(CCSelectedEnt)
			net.SendToServer()

			GAMEMODE:AddChat("You gave " .. CCSelectedEnt:VisibleRPName() .. " " .. val .. " dollars.", Color(200, 200, 200, 255))

		else

			GAMEMODE:AddChat("You don't have this many dollars!", Color(200, 0, 0, 255))

		end

	end
	CCP.GiveMoney.OK:PerformLayout()

	CCP.GiveMoney.Entry.OnEnter = CCP.GiveMoney.OK.DoClick
end
...
function GM:CCCreatePlayerViewer(ent)
	...
	compound.SetupModelPanel(CCP.PlayerViewer.CharacterModel, ent)
...
function GM:CreateCCContext(ent)
	...
	if IsValid(wep) and wep.GetContextOptions then
		for k, v in pairs(wep:GetContextOptions()) do
			menu:AddOption(v.Name, function()
				v.Func(wep, LocalPlayer())

				net.Start("nUseWeapon")
					net.WriteEntity(wep)
					net.WriteInt(k, 8)
				net.SendToServer()
			end)

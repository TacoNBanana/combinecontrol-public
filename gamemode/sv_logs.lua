...
function GM:SetupDataDirectories()
	...
	file.CreateDir("combinecontrol/savedprops/v2")
...
net.Receive("nGetRosterList", function(len, ply)
	if not ply:IsAdmin() then return end
	...
	if id == 1 then
		...
		GAMEMODE.SQL:Query([[
			SELECT SteamID, RPName, CombineFlag, LastOnline FROM $chars
				WHERE CombineFlag != '' AND Deleted = 0
			]], cb)
		...
		GAMEMODE.SQL:Query([[
			SELECT SteamID, RPName, CharFlags, LastOnline FROM $chars
...
function GM:LogMoney(text, ply)
	local ins = os.date("!%H:%M:%S") .. "\t" .. ply:SteamID() .. "\t" .. text
	self:LogFile("money", ins)

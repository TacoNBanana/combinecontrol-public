...
GM:RegisterLogType("admin_givemoney", LOG_ADMIN, function(data)
	local str = data.Amount > 0 and "%s gave %s dollars to %s" or "%s took %s dollars from %s"

	return string.format(str, GAMEMODE:FormatPlayer(data.Admin), math.abs(data.Amount), GAMEMODE:FormatCharacter(data.Char))
end)

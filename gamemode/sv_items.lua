...
function GM:DBCreateItem(classname, location, locationarg, callback)
	...
	local function cb(res)
		local item = GAMEMODE:LoadItem(res[1].id, classname, {}, location, locationarg, false)
		item:OnCreated()
...
function GM:DBLoadWorldItems()
	local function cb(res)
		for _, v in pairs(res) do
			...
			self:LoadItem(v.id, v.ItemClass, util.JSONToTable(v.CustomData), v.StorageType, pos, v.Deleted)
		end
	end

...
function GM:DBCreateItem(classname, location, locationarg, callback)
	...
	local charID = (location == ITEM_PLAYER) and locationarg or nil

	local worldX = (location == ITEM_WORLD) and locationarg.x or nil
	local worldY = (location == ITEM_WORLD) and locationarg.y or nil
	local worldZ = (location == ITEM_WORLD) and locationarg.z or nil

	self.SQL:Insert("$items", {ItemClass = classname, StorageType = location, CharacterID = charID, WorldX = worldX, WorldY = worldY, WorldZ = worldZ}, cb)
end

GM.TempItemIndex = GM.TempItemIndex or -1

function GM:CreateTempItem(classname, location, locationarg)
	if not class.Exists(classname) then
		return
	end

	local item = GAMEMODE:LoadItem(self.TempItemIndex, classname, {}, ITEM_NONE, nil, false)

	item:OnCreated()
	item:SetItemLocation(location, locationarg)

	self.TempItemIndex = self.TempItemIndex - 1
...
function GM:DBLoadItems(ids)
	...
	local function cb(res)
		for _, v in pairs(res) do
			...
			self:LoadItem(v.id, v.ItemClass, util.JSONToTable(v.CustomData) or {}, location, locationarg, v.Deleted)
		end
	end
...
function GM:DBLoadWorldItems()
	local function cb(res)
		for _, v in pairs(res) do
			...
			local ang = Angle(v.WorldAP, v.WorldAY, v.WorldAR)

			self:LoadItem(v.id, v.ItemClass, util.JSONToTable(v.CustomData), v.StorageType, {pos, ang, v.WorldFrozen}, v.Deleted)
		end
	end
...
function GM:DBLoadCharacterItems(charid)
	local function cb(res)
		for _, v in pairs(res) do
			self:LoadItem(v.id, v.ItemClass, util.JSONToTable(v.CustomData), v.StorageType, v.CharacterID, v.Deleted)
		end
	end

	self.SQL:Query([[
		SELECT * FROM $items
			WHERE StorageType = ? AND CharacterID = ? AND Deleted = 0
...
function GM:CreateItem(ply, classname, arguments, temp)
	if temp then
		...
		local item = self:CreateTempItem(classname, ITEM_WORLD, GAMEMODE:GetItemDropLocation(ply))
